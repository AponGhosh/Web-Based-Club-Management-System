<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        $imgone = $_FILES['image1'];
        $pName = $imgone['name'];
        $tempNameone = $imgone['tmp_name'];
        ...
        // Prepare and execute the SQL query
        $formatone = explode('.', $pName);
        $actualNameone = strtolower($formatone[0]);
        $actualFormatone = strtolower($formatone[1]);
        $allowedFormatsone = ['jpg', 'png', 'jpeg', 'gif'];

        if (in_array($actualFormat, $allowedFormats)) {
            $file = 'Uploads/' . $actualName . '.' . $actualFormat;
            $fileone = 'Uploads/' . $actualNameone . '.' . $actualFormatone;

            $sql = "INSERT INTO recent_events(event_ID, title, description, date, venue, imageone, imagetwo) VALUES ('$eventID', '$title', '$description', '$date', '$venue', '$file', '$fileone')";
            if ($conn->query($sql) === true) {
                move_uploaded_file($tempName, $file);
                move_uploaded_file($tempNameone, $fileone);
...
<header class="head">
    ...
    <form class="recent" action="recentevents.php" method="post" enctype="multipart/form-data">
        <div class="form-row">
        ...
        <div class="form-row">
            ...
            <div class="form-group col-md-6">
                <label for="image1">Upload Image One:</label>
