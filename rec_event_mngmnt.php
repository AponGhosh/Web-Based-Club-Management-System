<?php
    ...
    $sqlz = "SELECT id, event_ID, title, description, date, venue, imageone, imagetwo FROM recent_events";
    $sqlreg = "SELECT Event_ID, Full_name, Email, Contact, Student_ID FROM events_reg_list";
    $result = mysqli_query($conn, $sql);
    $data = mysqli_query($cone, $sqly);
    $info = mysqli_query($cone, $sqlz);
    $registration = mysqli_query($cone, $sqlreg);
...
<head>
    ...
    <br><br><br><br>
        <div class="hero">
            <h1 class="title"><b>Recent Events</b></h1><br><br>
        </div>
        <a href="http://localhost/Design-Project/recentevents.php">
        ...
        <table>
            ...
            if ($info->num_rows > 0) {
                while ($row = $info->fetch_assoc()) {
                    echo "<tr>";
                    // Add image column
                    echo "<td><img src='" . $row["imageone"] . "'></td>";
                    echo "<td><img src='" . $row["imagetwo"] . "'></td>";
                    echo "<td>" . $row["event_ID"] . "</td>";
                    echo "<td>" . $row["title"] . "</td>";
                    echo "<td>" . $row["description"] . "</td>";
                    echo "<td>" . $row["date"] . "</td>";
                    echo "<td>" . $row["venue"] . "</td>";
                    echo "<td><a href='deleterecentevents.php?id=" . $row['id'] . "' class='custom-delete-button'>Delete</a></td>";
